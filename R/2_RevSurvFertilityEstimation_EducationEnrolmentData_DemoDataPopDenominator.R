...
###        Using DemoData census data for Women
...
series_unesco <- 
  pop_females$SeriesID_UNESCO %>% unique
...
RevSurvEstimates <- 
  lapply( series_unesco, function( x ){
    ...
    for( s_fem in aux_f1$SeriesID %>% unique ){
      ...
      # 6) run function
      print( paste0( 'Country: ', loc, ' - SeriesID: ', x ) )
      outRevSurv <- 
        rbind(
          outRevSurv,
          data.table(
...
write.table( outRevSurv, 'outputs/reverse_survival_fertestr_world_unesco_demodatadenominator.csv', 
             row.names = F )
##################################################
