...
outRevSurvx5_graduated <- 
  fread( 'outputs/reverse_survival_fertest_world_x5_graduated_wppfunction.csv' ) %>%
  .[ !( as.numeric( SeriesID ) %in% manual_rmv_ids_x5 ) ]

outRevSurvx5 <-
  fread( 'outputs/reverse_survival_fertest_world_x5_wppfunction.csv' ) %>%
  .[ !( as.numeric( SeriesID ) %in% manual_rmv_ids_x5 ) ]


outRevSurvx1 <- 
  fread( 'outputs/reverse_survival_fertest_world_x1_wppfunction.csv' )


outRevSurvUnesco <- 
  fread( 'outputs/reverse_survival_fertest_world_unesco_popwpp_wppfunction.csv' )


data('tfr')

myLocations <- 
  c( outRevSurvx5$LocID,
     outRevSurvx1$LocID,
...
plot_all <- 
  rbind(
    ...
    plot_outx5[, .( SeriesID, LocID, name, year, TFR, TypeEst = 'RevSurv-x5' ) ],
...
pdf( file = 'figs/outputs_revsurv_world_all_wpp_func.pdf', 
     width = 8, height = 6 )
for( loc in sort( plot_all$LocID %>% unique ) ){
  ...
  plot_this <- 
    ...
    scale_color_manual( values = c( 'RevSurv-x1'  = 'black',
                                    'RevSurv-x5'  = 'gold',
                                    'Abridged-BeersModified' = 'skyblue',
                                    'Abridged-Sprague'       = 'forestgreen',
                                    'Educ. Enrolment'        = 'gray55',
                                    'WPP 2019'         = 'tomato3' ),
                        name = '' ) +
    scale_shape_manual( values = c( 'RevSurv-x1' = 1,
                                    'RevSurv-x5' = 19,
                                    'Abridged-BeersModified' = 3,
                                    'Abridged-Sprague'       = 4,
                                    'Educ. Enrolment'        = 18,
                                    'WPP 2019'               = 20 ),
                        name = '' ) +
    scale_linetype_manual( values = c( 'RevSurv-x1' = 'blank',
                                       'RevSurv-x5' = 'blank',
                                       'Abridged-BeersModified' = 'blank',
