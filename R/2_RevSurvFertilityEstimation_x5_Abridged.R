...
###        Using children 5 year age groups
###        Without Graduation  
###        WPP function version
...
RevSurvEstimatesX5 <- 
  lapply( series, function( x ){
    ...
    nLx0_4_c <- 
      getLifeTableWPP( locations = loc, year = ( date_ref_dec - 2.5 ), sex = 'Total' ) %>%
    ...
    nLx5_9_c <- 
      getLifeTableWPP( locations = loc, year = ( date_ref_dec - 7.5 ), sex = 'Total' ) %>%
      .[ , age5 := AgeStart - AgeStart %% 5 ] %>%
      .[ age5 %in% c( 0, 5, 10 ), 
         .( Lx = sum( Lx ) ),
         .( age5 ) ] %>%
    ...
    nLx10_14_c <- 
      getLifeTableWPP( locations = loc, year = ( date_ref_dec - 12.5 ), sex = 'Total' ) %>%
      .[ , age5 := AgeStart - AgeStart %% 5 ] %>%
      .[ age5 %in% c( 0, 5, 10 ), 
         .( Lx = sum( Lx ) ),
         .( age5 ) ] %>%
...
write.table( outRevSurvx5, 
             file = 'outputs/reverse_survival_wppfunction_world_x5_abridged.csv', 
             row.names = F )
